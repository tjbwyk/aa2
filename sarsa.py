...
def compute_q_value(Q_value, old_state, old_action, new_action, reward, new_state, predator, learning_rate=0.1,
                    discount_factor=0.9):
...
def run(learning_rate=0.1, discount_factor=0.9, epsilon=0.1, tau=0.1, value_init=15, num_episodes=1000, verbose=True,
    ...
    for i in range(1, num_episodes):
        ...
        while not field.is_ended():
            ...
            predator.policy.q_value[old_state, old_action] = compute_q_value(field.get_predator().policy.q_value,
                                                                             old_state, old_action, new_action, reward,
                                                                             new_state, predator, learning_rate,
                                                                             discount_factor)
    ...
    if gui:
        ...
        while not field.is_ended():
            field.act(style=policy_style, epsilon=epsilon, tau=tau)
            GUI.update()
            time.sleep(0.2)
