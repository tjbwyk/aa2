...
if __name__ == '__main__':
    ...
    average_window = 500
    learning_algo = ["qlearning","sarsa"] #"sarsa","qlearning"
    ...
    for args in it.product(*arg_list):
        ...
        plt.plot(avg,alpha=0.5,label="%s: $\\alpha=$%s, $\\gamma=$%s. $\\epsilon=$%s, initial value=%s, $\\tau =$%s, $\\pi=$%s" % ((args[7],)+ args[0:4] + args[5:7]))
